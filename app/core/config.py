...
def _slack_output_constraints(*, general: bool = False, expense: bool = False) -> str:
    if expense:
        ...
        ask_rule = (
            "미지원 요청이면 불가만 말하고 되묻지 않는다. "
            "지원 기능인데 필수 정보가 없을 때만 한 가지를 짧게 묻는다."
        )
    else:
...
def _connbot_supported_tools_block() -> str:
    return (
        "할 수 있는 일(이 목록만 가능, 그 외는 불가):\n"
        "- 사내 위키 검색·요약\n"
        "- Flex 근태 조회(당일·월간·특정일 브리핑)\n"
        "- 정부·지원사업 공고 브리핑\n"
        "- Outlook 회의실 조회·예약·취소·변경·리마인더(본인 주최·미래)\n"
        "- 본인 Outlook 일정 조회·생성·변경·취소\n"
        "- 경비 증빙 첨부 업로드\n"
        "- 인사·스몰토크·기능 안내·일반 상식·첨부 텍스트 단순 가공\n"
    )


def _connbot_unsupported_tools_block() -> str:
    return (
        "할 수 없는 일: 위 목록에 없는 요청은 전부 불가. 대표 예시는 다음과 같다.\n"
        "- 슬랙 채널·DM·스레드·캔버스 조회·요약·검색·메시지 작성·초대·권한 변경\n"
        "- 웹·인터넷 검색, 외부 사이트·뉴스·주식·날씨 실시간 조회\n"
        "- Flex 일정·휴가·근무 등록·변경·승인, 인사 발령·급여·평가 처리\n"
        "- 타인 Outlook 전체 일정 열람·조작, 과거 회의실 예약 변경, 메일·알림·외부 발송 대행\n"
        "- 위 목록에 없는 쓰기·대행·자동화 일체\n"
        "원칙: 가능 목록에 없으면 '제가 할 수 없습니다'로 단정하고, "
        "가능하다고 오해할 재질문·실행 약속·우회 시도 안내를 하지 않는다.\n"
    )


def _connbot_capabilities_block() -> str:
    return (
        "<cornbot_capabilities>\n"
...
def get_no_hedging_instructions(*, general: bool = False) -> str:
    ...
    unsupported = (
        "- 미지원 기능은 첫 줄에 '제가 할 수 없습니다'로 분명히 말한다. "
        "기간·형식·채널명을 되묻거나 '조회해 드리겠습니다'처럼 가능하다고 오해할 약속을 하지 않는다.\n"
        if general
        else ""
    )
    return (
...
def get_routing_policy() -> str:
    ...
    return (
        ...
        "도메인만 바뀐 생략형이면 conversation_context·session_agent_slots의 "
        "**사람·날짜·시각·회의실**을 새 tool 인자로 이어 받고 respond_general로 되묻지 않는다.\n"
        "가용 조회 직후 회의실만 고르는 예약('Atlas 1시간 예약해줘')은 직전 관심 시각을 "
        "start_time으로 채운 book이다. 'N시간'은 소요시간이지 시계 시각이 아니다.\n"
        "예약 직후 초대·참석자 추가('초대메일 보내'·'소연님 초대해')는 "
        "manage_room_schedule modify(attendees)다. 근태 조회(search_worker_schedule)로 바꾸지 않는다.\n"
        "사람 이름만 있는 질문은 도메인 전환 단서로 보지 않는다.\n\n"

        "[금지]\n"
        "- 생략형 follow-up이 아닌데 직전 도메인을 억지로 유지하는 것. 현재 질문 우선.\n"
        "- 맥락에 사람·대상·시각이 있는데 respond_general로 되묻는 것.\n"
        "- 사람 이름만 보고 근태 조회로 전환하는 것.\n"
        "- 회의 초대 요청을 Flex 근무 조회로만 처리하는 것.\n\n"

        "Turn1에서 바로 실행, 재확인 질문 금지\n"
        "- [회의실 조회] '모든/전체 회의실'·예약 현황 → check_all(date). list는 사람 기준 일정만.\n"
...
def get_turn1_routing_instruction() -> str:
    ...
    return (
        ...
        "[manage_room_schedule] 조회 action: "
        "check_all='모든/전체 회의실'·'회의실 예약 현황'·회의실별 occupied "
        "(date만, 시각 미지정=하루 전체; person_name 금지; list 금지). "
        "list=본인 또는 person_name(예: 소연) 주최·참석 회의실 일정 "
        "('내 회의'·'OO님 미팅'; '모든 회의실' 질문에는 list 쓰지 않음). "
        "check=room_name 한 곳. "
        "book subject 생략 가능('회의'). "
        "가용 조회 follow-up 예약은 session_agent_slots·conversation_context의 "
        "date/start_time/end_time을 book 인자에 넣는다. 'N시간'은 소요(+1h 기본).\n"
        "초대·참석자 추가는 modify + attendees(이름 가능). 근태 tool 금지. "
        "cancel/modify/replace/set_reminder: session_agent_slots의 booking_id를 "
        "tool 인자에 넣고(사용자에게 ID를 묻거나 말하지 않음), "
        "room/date/start 힌트도 함께 전달. "
        "list 기간: 이번 주·다음 주는 date+end_date. "
...
def get_router_general_final_instruction(
    *,
    ...
    return (
        ...
        "- **미지원 요청**(<cornbot_capabilities> 가능 목록에 없는 모든 요청): "
        "첫 줄에 '지금은 할 수 없습니다'만 분명히 말한다. "
        "기간·요약 형식·채널명 되묻기, '조회해 드리겠습니다'·'실행해 드릴까요' 등 "
        "가능하다고 오해할 안내·약속·우회 시도 금지. 지원 기능으로의 대안이 있으면 한 문장만 제안한다.\n"
        "- **지원되는 조회**(회의·근태·정부과제·위키)인데 이번 턴 tool이 실행되지 않은 경우만: "
        "'이번 턴에서 해당 조회가 실행되지 않았습니다' 한 문장. "
        "이미 채널·기간 등 필수 정보가 있으면 되묻지 않는다. 빠진 필수 슬롯이 있을 때만 한 가지를 짧게 묻는다. "
        "'구체화하면 조회해 드리겠습니다'처럼 아직 실행하지 않은 작업을 약속하지 않는다.\n"
    ...
    *,
    ...
    if has_context:
        return (
            ...
            "찾지 못했을 때도 예약 ID를 묻지 말고 회의실·날짜·시각만 확인한다. "
